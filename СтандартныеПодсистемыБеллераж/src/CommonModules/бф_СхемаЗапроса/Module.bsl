...
Функция ЗапросПакетаИзТаблицы(СхемаЗапроса, Знач ТаблицаИсточника, Знач ПсевдонимИсточника = "", Знач ВыбираемыеКолонки = Неопределено, ТаблицаДляПомещения = "", ЗапросНаУничтожение = Ложь, ВыбиратьРазрешенные = Ложь, ОператорВыбрать = Неопределено, Очистить = Ложь) Экспорт
	...
	Если ТипЗначения = Тип("Структура") Тогда
		...
		ЗапросПакета 	= бф_СхемаЗапроса.ЗапросПакета(СхемаЗапроса, ТаблицаДляПомещения);
		ОператорВыбрать = бф_СхемаЗапроса.Оператор(ЗапросПакета);
		Источник = бф_СхемаЗапроса.Источник(ОператорВыбрать, ТаблицаИсточника, ПсевдонимИсточника);
		...
		//		Продолжить;
		//	КонецЕсли;
		//	бф_СхемаЗапроса.Поле(ЗапросПакета, ОператорВыбрать,, ?(ПустаяСтрока(ПсевдонимИсточника), "", ПсевдонимИсточника+".")+Поле.Имя, Поле.Имя);
		//КонецЦикла;
	Иначе
		...
		Если ПустаяСтрока(ПсевдонимИсточника) Тогда
			...
			КонецЕсли;
		КонецЕсли;
		ЗапросПакета 	= бф_СхемаЗапроса.ДобавитьЗапросПакета(СхемаЗапроса, ТаблицаДляПомещения);
		ОператорВыбрать = бф_СхемаЗапроса.ДобавитьОператор(ЗапросПакета);
		Источник = бф_СхемаЗапроса.Источник(ОператорВыбрать, ТаблицаИсточника, ПсевдонимИсточника);
		...
		//		Продолжить;
		//	КонецЕсли;
		//	бф_СхемаЗапроса.Поле(ЗапросПакета, ОператорВыбрать,, ПсевдонимИсточника+"."+Поле.Псевдоним, Поле.Псевдоним);
		//КонецЦикла;
	КонецЕсли;
	...
	Если ВыбираемыеКолонки <> Неопределено Тогда
		...
		Если ТипВыбираемыеКолонки = Тип("Строка") 
			...
			Для каждого Колонка Из бф_СхемаЗапросаСлужебный.Массив(ВыбираемыеКолонки) Цикл
		...
		Иначе
			...
			КонецЦикла;
		КонецЕсли;
		//Для каждого ПутьКПолю Из бф_СхемаЗапросаСлужебный.Массив(ВыбираемыеКолонки) Цикл
	...
	Иначе
		Для Индекс = 0 По МассивПараметров.ВГраница() Цикл
			...
			Если ПустаяСтрока(Выражение) Тогда
				Продолжить;
			КонецЕсли;
			СтруктураПараметров.Вставить(МассивПараметров[Индекс], бф_СхемаЗапросаСлужебный.ЭкранироватьТекст(Выражение));
		КонецЦикла;
	КонецЕсли;
...
Функция ОписаниеВременнойТаблицы(ИмяТаблицы, ИсточникДанных = Неопределено, КоллекцияКолонок = Неопределено) Экспорт
	Если ТипЗнч(КоллекцияКолонок) = Тип("Строка") Тогда
		мКолонки = бф_СхемаЗапросаСлужебный.Массив(КоллекцияКолонок);
	Иначе
...
//					- Структура - Структуру можно получить из сервисных функций (см. функцию бф_СхемаЗапроса.ПолучитьОписаниеВложенногоЗапроса(), бф_СхемаЗапроса.ПолучитьОписаниеВременнойТаблицы(ИмяТаблицы)).
	...
	Иначе
		...
		Если ТипЗнч(Таблица) = Тип("Строка") Тогда
			...
			Если ПустаяСтрока(Псевдоним) Тогда
				Псевдоним = СтрЗаменить(бф_СхемаЗапросаСлужебный.ОкончаниеСтрокиПослеРазделителя(Таблица), "&", "");
			КонецЕсли;
		Иначе
			Если ПустаяСтрока(Псевдоним) Тогда
				Если Таблица = Тип("ОписаниеВременнойТаблицыСхемыЗапроса") Тогда
					Псевдоним = бф_СхемаЗапросаСлужебный.ИмяПоУникальномуИдентификатору(Новый УникальныйИдентификатор);
				КонецЕсли;
			КонецЕсли;
...
// Примеры:
//  Новое поле:
//		бф_СхемаЗапроса.Поле(ЗапросПакета, ОператорВыбрать,, "Документ.Контрагент", "Контрагент");
// 	Установка поля или подмена в копии оператора, можно подменить путь или выражение для колонки запроса:
//		бф_СхемаЗапроса.Поле(ЗапросПакета, ОператорВыбрать,, "ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)", "Контрагент");
//
...
//		бф_СхемаЗапроса.ВыражениеПоля(ОператорВыбрать, "СУММА", "Документ.СуммаДокумента");
// 	Пример для подмены, можно подменить на существующее поле или выражение:
//		бф_СхемаЗапроса.ВыражениеПоля(ОператорВыбрать,, "Документ.Контрагент", "ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)");
//		бф_СхемаЗапроса.ВыражениеПоля(ОператорВыбрать,, "Документ.Контрагент", "Документ.Организация");
...
//
// Пример:
//  бф_СхемаЗапроса.ИзменитьПсевдонимКолонки(ЗапросПакета, "СуммаОстаток", "СуммаРазмещено");
//
...
Функция УсловиеСоединения(Поля) Экспорт
	...
	Если ТипЗнч(Поля) = Тип("Строка") ИЛИ ТипЗнч(Поля) = Тип("Массив") Тогда
		Для каждого Поле Из бф_СхемаЗапросаСлужебный.Массив(Поля) Цикл
...
Функция УсловиеВхождения(Источник, ИсточникДанных = Неопределено, Поля = "*") Экспорт
	...
	Если Поля = "*" Тогда
		...
		КонецЕсли;
	Иначе
		МассивПолей = бф_СхемаЗапросаСлужебный.Массив(Поля);
	КонецЕсли;
	...
	КонецЦикла;
	Если ЗначениеЗаполнено(Источник) Тогда
		ЗапросПакета = бф_СхемаЗапроса.ЗапросПакетаИзТаблицы(СхемаЗапроса, бф_СхемаЗапроса.ОписаниеВременнойТаблицы(Источник, ИсточникДанных, МассивПолейСправа),, МассивПолейСправа,,,, ОператорВыбрать);
		ОператорВыбрать.ВыбиратьРазличные = Истина;
	Иначе
		ЗапросПакета = бф_СхемаЗапроса.ЗапросПакета(СхемаЗапроса,,, ОператорВыбрать);
		Для Индекс = 0 По МассивПолейСлева.ВГраница() Цикл
			бф_СхемаЗапроса.Поле(ЗапросПакета, ОператорВыбрать,, МассивПолейСправа[Индекс], МассивПолейСлева[Индекс]);
		КонецЦикла;
	КонецЕсли;
...
//
// Пример:
//	бф_СхемаЗапроса.ДобавитьСоединениеХарактеристики(ОператорВыбрать, "ВТ_ДВИЖЕНИЯ_ОПЕРАЦИИ", "СтатьяИсточникАналитика1",
...
Процедура ДобавитьСоединениеХарактеристики(Оператор, Знач ИсточникОбъекта, Знач ИсточникХарактеристики, ПолеОбъектаИсточника = "Ссылка", 
	...
	бф_СхемаЗапроса.ДобавитьСоединение(,
		ИсточникОбъекта,
...
Процедура ДобавитьУсловиеХарактеристики(Оператор, Источник, ИсточникЗначений, ПолеОбъектаИсточника = "Ссылка", ПолеОбъекта = "Объект", ПолеВида = "Свойство", ПолеЗначения = "Значение", ВыражениеПараметраВида = "", ВыражениеПараметраЗначения = "") Экспорт 
	Если ПустаяСтрока(ВыражениеПараметраВида) Тогда
		ВыражениеПараметраВида 		= "&"+бф_СхемаЗапросаСлужебный.ИмяПоУникальномуИдентификатору(Новый УникальныйИдентификатор);
	КонецЕсли;
	Если ПустаяСтрока(ВыражениеПараметраЗначения) Тогда
		ВыражениеПараметраЗначения 	= "&"+бф_СхемаЗапросаСлужебный.ИмяПоУникальномуИдентификатору(Новый УникальныйИдентификатор);
	КонецЕсли;
	ПсевдонимИсточника			= Источник.Источник.Псевдоним;
	ПсевдонимИсточникаЗначений	= ПсевдонимИсточника+ВыражениеПараметраВида;
	бф_СхемаЗапроса.ДобавитьИсточник(Оператор, 
		ИсточникЗначений,
...
//
// Пример:
//	бф_СхемаЗапроса.ДобавитьУсловиеОграниченияПоВидуДоступа(СхемаЗапроса, ОператорИсточника,, "ЦФО");
...
Процедура ДобавитьУсловиеОграниченияПоВидуДоступа(СхемаЗапроса, ОператорИсточника, ПсевдонимИсточника = "Источник", ВидДоступа) Экспорт
	////////////////////////////////////////////////////////////////////////////////
	//  ЗАПРОС ПАКЕТА 0
	ЗапросПакета 	= бф_СхемаЗапроса.ДобавитьЗапросПакета(СхемаЗапроса, "ВТ_ЗНАЧЕНИЯ_ДОСТУПА");
	ОператорВыбрать = бф_СхемаЗапроса.ДобавитьОператор(ЗапросПакета, Новый Структура("ВыбиратьРазличные", Истина));
	бф_СхемаЗапроса.ДобавитьИсточник(ОператорВыбрать, "Справочник.Пользователи"					, "СправочникПользователи");
	бф_СхемаЗапроса.ДобавитьИсточник(ОператорВыбрать, "Справочник.ГруппыДоступа");
	бф_СхемаЗапроса.ДобавитьИсточник(ОператорВыбрать, "Справочник.ГруппыДоступа.ЗначенияДоступа"	, "ГруппыДоступаЗначенияДоступа");
	бф_СхемаЗапроса.ДобавитьИсточник(ОператорВыбрать, "Справочник.ГруппыДоступа.ВидыДоступа"		, "ГруппыДоступаВидыДоступа");
	бф_СхемаЗапроса.ДобавитьСоединение(ОператорВыбрать, "СправочникПользователи"		, "ГруппыДоступа"				, "СправочникПользователи.Ссылка = ГруппыДоступа.Пользователь", ТипСоединенияСхемыЗапроса.Внутреннее);
	бф_СхемаЗапроса.ДобавитьСоединение(ОператорВыбрать, "ГруппыДоступа"				, "ГруппыДоступаЗначенияДоступа", "ГруппыДоступа.Ссылка = ГруппыДоступаЗначенияДоступа.Ссылка", ТипСоединенияСхемыЗапроса.Внутреннее);
	бф_СхемаЗапроса.ДобавитьСоединение(ОператорВыбрать, "ГруппыДоступаЗначенияДоступа", "ГруппыДоступаВидыДоступа"	, 
	...
	бф_СхемаЗапроса.Поле(ЗапросПакета, ОператорВыбрать,, "ГруппыДоступаЗначенияДоступа.ВидДоступа");
	бф_СхемаЗапроса.Поле(ЗапросПакета, ОператорВыбрать,, "ГруппыДоступаВидыДоступа.ВсеРазрешены");
	бф_СхемаЗапроса.Поле(ЗапросПакета, ОператорВыбрать,, "ГруппыДоступаЗначенияДоступа.ЗначениеДоступа");
	//  Переместить запрос пакета в начало
	бф_СхемаЗапроса.УстановитьИндексЗапросаПакета(СхемаЗапроса, ЗапросПакета, 0);
	////////////////////////////////////////////////////////////////////////////////
	//  ЗАПРОС ПАКЕТА 1
	бф_СхемаЗапроса.ДобавитьИсточник(ОператорИсточника, "ВТ_ЗНАЧЕНИЯ_ДОСТУПА", "ВТ_ЗНАЧЕНИЯ_ДОСТУПА_ИСКЛЮЧИТЬ");
	бф_СхемаЗапроса.ДобавитьИсточник(ОператорИсточника, "ВТ_ЗНАЧЕНИЯ_ДОСТУПА", "ВТ_ЗНАЧЕНИЯ_ДОСТУПА_ВКЛЮЧИТЬ");
	бф_СхемаЗапроса.ДобавитьСоединение(ОператорИсточника, ПсевдонимИсточника, "ВТ_ЗНАЧЕНИЯ_ДОСТУПА_ИСКЛЮЧИТЬ", ПсевдонимИсточника+".Ссылка = ВТ_ЗНАЧЕНИЯ_ДОСТУПА_ИСКЛЮЧИТЬ.ЗначениеДоступа");
	бф_СхемаЗапроса.ДобавитьСоединение(ОператорИсточника, ПсевдонимИсточника, "ВТ_ЗНАЧЕНИЯ_ДОСТУПА_ИСКЛЮЧИТЬ", "ВТ_ЗНАЧЕНИЯ_ДОСТУПА_ИСКЛЮЧИТЬ.ВсеРазрешены");
	бф_СхемаЗапроса.ДобавитьСоединение(ОператорИсточника, ПсевдонимИсточника, "ВТ_ЗНАЧЕНИЯ_ДОСТУПА_ВКЛЮЧИТЬ" , ПсевдонимИсточника+".Ссылка = ВТ_ЗНАЧЕНИЯ_ДОСТУПА_ВКЛЮЧИТЬ.ЗначениеДоступа");
	бф_СхемаЗапроса.ДобавитьСоединение(ОператорИсточника, ПсевдонимИсточника, "ВТ_ЗНАЧЕНИЯ_ДОСТУПА_ВКЛЮЧИТЬ" , "НЕ ВТ_ЗНАЧЕНИЯ_ДОСТУПА_ВКЛЮЧИТЬ.ВсеРазрешены");
...
//		бф_СхемаЗапроса.Итог(ЗапросПакета, "Контрагент");
//		ЗапросПакета.ВыраженияИтогов.Добавить("СУММА(СуммаУпр)", "СуммаУпр");
//	или можно через выражение:
//		бф_СхемаЗапроса.Итог(ЗапросПакета, "Контрагент.ИНН+Контрагент.КПП", "ИННКПП");
//		ЗапросПакета.ВыраженияИтогов.Добавить("СУММА(СуммаУпр)", "СуммаУпр");
//
// Параметры:
//  ЗапросПакета - ЗапросСхемыЗапроса
...
Функция Итог(ЗапросПакета, Выражение, ИмяКолонки = "", ТипКонтрольнойТочки = Неопределено, ТипДополненияПериодами = Неопределено, НачалоПериода = "", КонецПериода = "") Экспорт
	...
	КонтрольнаяТочка.НачалоПериодаДополнения 	= ?(ТипЗнч(НачалоПериода) 	= Тип("Строка"), НачалоПериода, бф_СхемаЗапросаСлужебный.ФорматДатаВремя(НачалоПериода));
	КонтрольнаяТочка.КонецПериодаДополнения 	= ?(ТипЗнч(КонецПериода) 	= Тип("Строка"), КонецПериода, бф_СхемаЗапросаСлужебный.ФорматДатаВремя(КонецПериода));
...
Функция ДобавитьЗапросПакетаИзТаблицы(СхемаЗапроса, Знач ТаблицаИсточника, Знач ПсевдонимИсточника = "", Знач ВыбираемыеКолонки = Неопределено, ТаблицаДляПомещения = "", ЗапросНаУничтожение = Ложь, ВыбиратьРазрешенные = Ложь, ОператорВыбрать = Неопределено) Экспорт
	...
	Если ТипЗначения = Тип("Структура") Тогда
		...
		ЗапросПакета 	= бф_СхемаЗапроса.ДобавитьЗапросПакета(СхемаЗапроса, ТаблицаДляПомещения);
		ОператорВыбрать = бф_СхемаЗапроса.ДобавитьОператор(ЗапросПакета);
		бф_СхемаЗапроса.ДобавитьИсточник(ОператорВыбрать, ТаблицаИсточника, ПсевдонимИсточника);
		Для каждого Поле Из ТаблицаИсточника.ДоступныеПоля Цикл
			бф_СхемаЗапроса.Поле(ЗапросПакета, ОператорВыбрать,, ?(ПустаяСтрока(ПсевдонимИсточника), "", ПсевдонимИсточника+".")+Поле.Имя, Поле.Имя);
		КонецЦикла;
	Иначе
...
Функция ДобавитьИсточник(Оператор, ОписаниеТаблицы, Псевдоним = "", ПараметрыВиртуальнойТаблицы = Неопределено) Экспорт
	...
	Иначе
		...
		Если ТипЗнч(Таблица) = Тип("Строка") Тогда
			ИмяТаблицы		= Таблица;
			Если ПустаяСтрока(Псевдоним) Тогда
				Псевдоним = бф_СхемаЗапросаСлужебный.ОкончаниеСтрокиПослеРазделителя(Таблица);
			КонецЕсли;
		Иначе
			Если ПустаяСтрока(Псевдоним) Тогда
				Если Таблица = Тип("ОписаниеВременнойТаблицыСхемыЗапроса") Тогда
