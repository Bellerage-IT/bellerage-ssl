...
Функция ВыражениеВхождения(ИмяТаблицы, Поля = "*", ПоляИсточника = Неопределено) Экспорт
	...
	Для Каждого Связь Из бф_СхемаЗапросаСлужебный.Массив(Поля) Цикл
...
Функция ЗапросПакета(ИмяЗапроса = "") Экспорт
	...
	Если НЕ ПустаяСтрока(ИмяЗапроса) Тогда
		бф_СхемаЗапроса.ОписаниеЗапроса(СхемаЗапроса, СтруктураЗапроса, ИмяЗапроса);
	КонецЕсли;
	Возврат ЭтотОбъект;
КонецФункции
...
Функция Уничтожить(ИмяВременнойТаблицы) Экспорт
	бф_СхемаЗапроса.ЗапросУничтожения(СхемаЗапроса, ИмяВременнойТаблицы);
	Возврат ЭтотОбъект;
КонецФункции
...
Функция Порядок(Выражение, Направление = Неопределено) Экспорт
	Если ТипЗнч(Направление) = Тип("Строка") Тогда
		Если Направление = "-" Тогда
			бф_СхемаЗапроса.Порядок(ЗапросПакета, Выражение, НаправлениеПорядкаСхемыЗапроса.ПоУбыванию);
			Возврат ЭтотОбъект;
		КонецЕсли;
		Если Направление = "+" Тогда
			бф_СхемаЗапроса.Порядок(ЗапросПакета, Выражение, НаправлениеПорядкаСхемыЗапроса.ПоВозрастанию);
			Возврат ЭтотОбъект;
		КонецЕсли;
		бф_СхемаЗапроса.Порядок(ЗапросПакета, Выражение, НаправлениеПорядкаСхемыЗапроса[Направление]);
		Возврат ЭтотОбъект;
	КонецЕсли;				
	бф_СхемаЗапроса.Порядок(ЗапросПакета, Выражение, Направление);
	Возврат ЭтотОбъект;
КонецФункции
...
Функция ПсевдонимКолонки(ТекущийПсевдоним, Псевдоним) Экспорт
	бф_СхемаЗапроса.ИзменитьПсевдонимКолонки(ЗапросПакета, ТекущийПсевдоним, Псевдоним);
	Возврат ЭтотОбъект;
КонецФункции
...
Функция Выбрать(Количество = Неопределено, Различные = Неопределено, ТипОбъединения = Неопределено, Индекс = Неопределено) Экспорт
	Если Индекс <> Неопределено Тогда
		...
		КонецЕсли;
		ОператорВыбрать = бф_СхемаЗапроса.КопияОператора(ЗапросПакета, ОператорИсточник);
	Иначе
...
Функция Поле(Выражение, Псевдоним = "", ВыражениеЕстьNull = "") Экспорт
	Если ПустаяСтрока(ВыражениеЕстьNull) Тогда
		Поле = бф_СхемаЗапроса.Поле(ЗапросПакета, ОператорВыбрать,, Выражение, Псевдоним);
		Возврат ЭтотОбъект;
	КонецЕсли;
	...
	Если ПустаяСтрока(ПсевдонимПоля) Тогда
		...
		КонецЕсли;
	КонецЕсли;
	Поле = бф_СхемаЗапроса.Поле(ЗапросПакета, ОператорВыбрать,, СтрШаблон("ЕСТЬNULL(%1, %2)", Выражение, ВыражениеЕстьNull), ПсевдонимПоля);
	Возврат ЭтотОбъект;
КонецФункции
...
Функция УдалитьПоле(Поле) Экспорт
	бф_СхемаЗапроса.УдалитьПоле(ОператорВыбрать, Поле);
	Возврат ЭтотОбъект;
КонецФункции

Функция УдалитьКолонку(Псевдоним) Экспорт
	бф_СхемаЗапроса.УдалитьКолонку(ЗапросПакета, Псевдоним);
	Возврат ЭтотОбъект;
КонецФункции
...
Функция Источник(Таблица, Знач Псевдоним = "", ПараметрыТаблицы = Неопределено) Экспорт
	...
	Если ТипЗнч(Таблица) <> Тип("Строка") Тогда
		ИмяПараметра = бф_СхемаЗапросаСлужебный.ИмяПоУникальномуИдентификатору();
		ВременнаяТаблица = бф_СхемаЗапроса.ОписаниеВременнойТаблицы("&" + ИмяПараметра, Таблица);
		Источник = бф_СхемаЗапроса.Источник(ОператорВыбрать, ВременнаяТаблица, Псевдоним);
	...
	Если СтрНачинаетсяС(Таблица, "&") Тогда
		...
		ВременнаяТаблица = бф_СхемаЗапроса.ОписаниеВременнойТаблицы("&" + ИмяПараметра, ПараметрыТаблицы);
		Источник = бф_СхемаЗапроса.Источник(ОператорВыбрать, ВременнаяТаблица, Псевдоним);
		ИсточникСлева = Источник;
	...
	Если МенеджерВременныхТаблиц.Таблицы.Найти(Таблица) <> Неопределено Тогда
		ВременнаяТаблица = бф_СхемаЗапроса.ОписаниеВременнойТаблицы(Таблица, МенеджерВременныхТаблиц);
		Источник = бф_СхемаЗапроса.Источник(ОператорВыбрать, ВременнаяТаблица, Псевдоним);
		ИсточникСлева = Источник;
		Возврат ЭтотОбъект;
	КонецЕсли;
	
	Источник = бф_СхемаЗапроса.Источник(ОператорВыбрать, Таблица, Псевдоним, ПараметрыТаблицы);
	ИсточникСлева = Источник;
	Возврат ЭтотОбъект;
КонецФункции

Функция ИсточникНачать(Псевдоним) Экспорт
	ВложенныйЗапрос = бф_СхемаЗапроса.Источник(ОператорВыбрать, бф_СхемаЗапроса.ОписаниеВложенногоЗапроса(), Псевдоним).Источник.Запрос;
...
Функция ЗаменитьИсточник(ИсходныйИсточник, Таблица, НовыйПсевдоним = "") Экспорт
	бф_СхемаЗапроса.ЗаменитьИсточник(ЗапросПакета, ОператорВыбрать, ИсходныйИсточник, Таблица, НовыйПсевдоним);
	Возврат ЭтотОбъект;
КонецФункции

Функция УдалитьИсточник(Псевдоним) Экспорт
	бф_СхемаЗапроса.УдалитьИсточник(ОператорВыбрать, Псевдоним);
	Возврат ЭтотОбъект;
КонецФункции

Функция ПараметрТаблицы(ИмяПараметра, Значение) Экспорт
	бф_СхемаЗапроса.УстановитьПараметрыВиртуальнойТаблицы(Источник, Новый Структура(ИмяПараметра, Значение));
	Возврат ЭтотОбъект;
КонецФункции
...
Функция Условие(Выражение) Экспорт
	...
	ПараметрыТаблицы = бф_СхемаЗапроса.СтруктураПараметровВиртуальнойТаблицы(Источник.Источник.ИмяТаблицы, Источник.Источник.Параметры);
...
Функция Связь(СоответствиеПолей) Экспорт
	...
	Если СоответствиеПолей = "*" Тогда
		...
		КонецЦикла;
		Выражение = бф_СхемаЗапроса.УсловиеСоединения(ОбщиеПоля);
	Иначе
		Выражение = бф_СхемаЗапроса.УсловиеСоединения(СоответствиеПолей);
	КонецЕсли;
	Если Соединение = Неопределено Тогда
		Соединение = бф_СхемаЗапроса.Соединение(ОператорВыбрать, ИсточникСлева, ИсточникСправа, Выражение, ТипСоединения);
		Возврат ЭтотОбъект;	
	КонецЕсли;
	...
	бф_СхемаЗапроса.ДобавитьУсловиеСоединения(Соединение, Выражение);		
	Возврат ЭтотОбъект;
КонецФункции
...
Функция УсловиеСвязи(Выражение) Экспорт
	...
	КонецЕсли;
	бф_СхемаЗапроса.ДобавитьУсловиеСоединения(Соединение, Условие);	
	Возврат ЭтотОбъект;
КонецФункции
...
Функция Группировка(Выражение, ИмяКолонки = "", ТипКонтрольнойТочки = Неопределено) Экспорт
	КонтрольнаяТочка = бф_СхемаЗапроса.Итог(ЗапросПакета, Выражение, ?(ПустаяСтрока(ИмяКолонки), Выражение, ИмяКолонки), ТипКонтрольнойТочки);
	Возврат ЭтотОбъект;
КонецФункции
...
Функция ВыполнитьЗапрос() Экспорт
	Запрос = бф_СхемаЗапроса.Запрос(СхемаЗапроса, Параметры, МенеджерВременныхТаблиц);
	ЗаполнитьЗначенияСвойств(ЭтотОбъект, СостояниеПоУмолчанию());
