...
Процедура Различные(
	Конвейер,
	Знач ФункцияРавенства = Неопределено,
	Знач ДополнительныеПараметры = Неопределено) Экспорт

	Если ФункцияРавенства = Неопределено Тогда
		ФункцияРавенства = бф_ПроцессорыКоллекций.СтандартнаяФункцияРавенства();
	Иначе
	...
	ПоложитьЯчейкуВКонвейер(Конвейер, "Различные", ФункцияРавенства);
...
Процедура ВыполнитьРазличные(Результат, ДополнительныеПараметры) Экспорт
	...
	Для Каждого Элемент Из ДополнительныеПараметры.Коллекция Цикл
		...
		Для Каждого ЭлементРезультат Из Результат Цикл
			...
			ВыполнитьОбработкуОповещенияСлужебный(ОбработчикСравнения, РезультатСравнения);
...
Процедура ОбработкаОповещения(Результат, ДополнительныеПараметры) Экспорт
	...
	// Развернуть доп параметры в локальные переменные
	Строки = Новый Массив;
	...
	Строки.Добавить(ПользовательскоеВыражение);
	
	//@skip-check server-execution-safe-mode
	Выполнить СтрСоединить(Строки, Символы.ПС);
...
Функция СформироватьВременноеОписаниеОповещения(
	Знач ПользовательскоеВыражение,
	ДополнительныеПараметры,
	...
	Если ПодставлятьСтрокуУстановкиРезультата И Не бф_РегулярныеВыраженияКлиентСервер.Совпадает(
			ПользовательскоеВыражение, "\s*Результат\s*=", , Истина) Тогда
