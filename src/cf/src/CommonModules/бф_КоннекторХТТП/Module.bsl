...
//    * Файлы - См. НовыйОтправляемыйФайл - файл к отправке
//    		  - Массив Из См. НовыйОтправляемыйФайл - файлы, к отправке
...
// Проверяет ответ, записывает "плохо" ответ в ЖР, по-умолчанию вызывает исключение
// 
// Параметры:
//  Ответ - см. НовыйОтвет
//  КодСобытия - Строка - Идентификатор события для записи в ЖР
//  ВызыватьИсключение - Булево - Признак вызова исключения есть ло
// 
// Возвращаемое значение:
//  Булево - Ответ ок
Функция ПроверитьОтвет(Ответ, КодСобытия = Неопределено, ВызыватьИсключение = Истина) Экспорт
	
	Если Ответ.КодСостояния < 400 Тогда	
		Возврат Истина;
	КонецЕсли;
	
	_КодСобытия = ?(КодСобытия = Неопределено, "КоннекторХТТП.ПлохойОтвет", КодСобытия);
	
	ТексОшибки = КакИсключение(Ответ);
